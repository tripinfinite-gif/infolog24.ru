import { NextResponse } from "next/server";
import { eq } from "drizzle-orm";
import { db } from "@/lib/db";
import { orders, payments } from "@/lib/db/schema";
import { getSession } from "@/lib/auth/session";
import { generateInvoiceHtml } from "@/lib/payments/invoice";
import { logger } from "@/lib/logger";

export const runtime = "nodejs";

const ORDER_TYPE_LABELS: Record<string, string> = {
  mkad_day: "Пропуск МКАД (дневной)",
  mkad_night: "Пропуск МКАД (ночной)",
  ttk: "Пропуск ТТК",
  sk: "Пропуск Садовое кольцо",
  temp: "Временный пропуск",
};

export async function GET(
  _request: Request,
  { params }: { params: Promise<{ id: string }> },
) {
  try {
    const session = await getSession();
    if (!session) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    const { id } = await params;

    // Find payment and related order
    const payment = await db.query.payments.findFirst({
      where: eq(payments.id, id),
      with: {
        order: {
          with: { user: true },
        },
      },
    });

    if (!payment) {
      return NextResponse.json(
        { error: "Payment not found" },
        { status: 404 },
      );
    }

    // Verify ownership (allow admins and managers too)
    const userRole = (session.user as Record<string, unknown>).role as string;
    if (
      payment.userId !== session.user.id &&
      userRole !== "admin" &&
      userRole !== "manager"
    ) {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }

    const order = payment.order;
    const user = order.user;
    const serviceDescription =
      ORDER_TYPE_LABELS[order.type] ?? `Оформление пропуска (${order.type})`;

    const html = generateInvoiceHtml({
      invoiceNumber: `INV-${payment.id.slice(0, 8).toUpperCase()}`,
      date: payment.createdAt.toLocaleDateString("ru-RU"),
      clientName: user.name ?? user.email,
      clientCompany: user.company ?? undefined,
      clientInn: user.inn ?? undefined,
      items: [
        {
          description: serviceDescription,
          quantity: 1,
          price: payment.amount,
        },
      ],
      total: payment.amount,
    });

    return new NextResponse(html, {
      status: 200,
      headers: {
        "Content-Type": "text/html; charset=utf-8",
      },
    });
  } catch (error) {
    logger.error(error, "Failed to generate invoice");
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 },
    );
  }
}
