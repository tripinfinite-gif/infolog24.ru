...
export async function POST() {
  ...
  try {
    ...
    facts.push(`Непрочитанных уведомлений: ${summary.counts.unreadAlerts}`);

    if (summary.recentAlerts.length > 0) {
      facts.push("Свежие непрочитанные алерты (сверху — самые новые):");
      for (const a of summary.recentAlerts.slice(0, 3)) {
        facts.push(`  - ${a.title}`);
      }
    }
...
Правила приветствия (приоритеты сверху вниз — выбери самую важную тему):
- ЕСЛИ есть свежие непрочитанные алерты (особенно permit_expiring_3days, permit_expiring_1day, rnis_silent) — обязательно упомяни их в первую очередь, конкретно и без паники.
- ЕСЛИ есть истекающие пропуска (без алертов) — упомяни самый ближайший с конкретной цифрой дней и предложи помочь с продлением.
